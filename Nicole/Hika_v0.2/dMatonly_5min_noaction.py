    ...
    def readMarket(self):
        ...
        for i in csvfile.index:
            ...
            if dt >= datetime.combine(self.startdate, time(18)) and dt <= datetime.combine(self.enddate,time(17,15)):
                if dt.time() >= time(18):
                    ...
                    openday = (dt-timedelta(1)).date()
                if openday in self.matdate:
        ...
        for dt in datetimeList:
            ...
            if lowDict[dt] <= short_exitprice:
                ...
                shortCF = 0.0
                if dt.time() >= time(18):
                    mat_high = self.matvalue[dt.date()][0]
                else:
                    mat_high = self.matvalue[(dt-timedelta(1)).date()][0]
            ...
            if highDict[dt] >= long_exitprice:
                ...
                longCF = 0.0
                if dt.time() >= time(18):
                    mat_low = self.matvalue[dt.date()][1]
                else:
                    mat_low = self.matvalue[(dt-timedelta(1)).date()][1]
...
if __name__ == "__main__":
    ...
    stoploss = 0.03
    startdate = datetime(2016,1,13).date()
    enddate = datetime(2016,4,12).date()
